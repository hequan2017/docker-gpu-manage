   ...
   <template #default="scope">
    ...
    <template #default="scope">
        <span>{{ filterDataSource(dataSource.userId,scope.row.userId) }}</span>
            ...
            <el-table-column align="left" label="关联ID" prop="relatedId" width="120" />

            <el-table-column align="left" label="关联类型(Model/Dataset/Space)" prop="relatedType" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button v-auth="btnAuth.edit" type="primary" link icon="edit" class="table-button" @click="updateMsDiscussionFunc(scope.row)">编辑</el-button>
    ...
    <el-input v-model.number="formData.relatedId" :clearable="false" placeholder="请输入关联ID" />
</el-form-item>
...
const getTableData = async() => {
  const table = await getMsDiscussionList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteMsDiscussionByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateMsDiscussionFunc = async(row) => {
    const res = await findMsDiscussion({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteMsDiscussionFunc = async (row) => {
    const res = await deleteMsDiscussion({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
